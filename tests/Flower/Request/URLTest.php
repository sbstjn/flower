...
class URLTest extends \PHPUnit_Framework_TestCase
{
    public function testParseFromEnvironment()
    {
        ...
        $url = \Flower\Request\URL::parseFromEnvironment($ENV);

        $this->assertInstanceOf('\Flower\Request\URL', $url);
    ...
    {
        $url = new \Flower\Request\URL('http://example.com/lorem?fancy=shit&lorem=ipsum+1');
        $this->assertEquals('http', $url->scheme());
        $this->assertTrue($url->http());
        $this->assertFalse($url->https());
